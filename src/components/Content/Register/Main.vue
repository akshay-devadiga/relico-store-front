...
<!--   class="mx-auto"         -->
      ...
      <v-window  v-model="step">
        <v-window-item :value="1">
           <span class="text-caption grey--text text--darken-1 mb-3">
              These details will be used as billing address
            </span>
          <v-card-text>
             {{errors}}
           <v-form ref="form" v-model="valid" lazy-validation>
          <v-text-field v-validate="'required'" name="name" v-model="name" label="Name" outlined></v-text-field>
          <v-text-field v-validate="'email'" name="email"  v-model="email" :rules="emailRules" label="E-mail" color="primary" outlined></v-text-field>
          <v-textarea v-validate="'required'" name="address"  label="Please add your billing address here" hint="Hint text" color="primary" outlined></v-textarea>
          <v-autocomplete solo outlined :items="countries" name="country" v-model="selectedCountry" item-text="name" label="Select Country" return-object></v-autocomplete>
          <v-autocomplete solo outlined :items="selectedCountry && selectedCountry.stateProvinces" name="province"  v-model="selectedProvince" item-text="name" label="Select State or Province" return-object></v-autocomplete>
          <v-text-field v-validate="'required'" type="number" name="contactno" v-model="contactNo" label="Contact No" outlined></v-text-field>
          <v-text-field v-validate="'required'" name="state"  v-model="stateOrProvince" label="State" color="primary" outlined></v-text-field>
          <v-text-field v-validate="'required'" name="town" v-model="town" label="Town" color="primary" outlined></v-text-field>
          <v-text-field v-validate="'required'" name="postalcode" v-model="postalcode" label="Postal Code" color="primary" outlined></v-text-field>
             <v-col cols=12 v-if="errors && errors.items.length>0">
                        <v-card class="ma-0" flat>
                          <v-card-title>
                            <p class="caption">Some of the details are missing. Please add them to continue</p>
                          </v-card-title>
                          <v-card-text class="px-6 pt-0">
                            <v-row no-gutters class="typography-widget pb-2">
                              <v-col cols="8" class="card-dark-grey ml-1" v-for="error in errors.items" :key="error">
                                          <v-alert
                                            dense
                                            outlined
                                            type="error"
